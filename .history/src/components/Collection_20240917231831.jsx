    ...
    <div className="collection bg-[#011631] flex flex-col px-6 w-full py-12 lg:px-12 gap-5 ">
      ...
      <div className="create-people flex flex-col gap gap-y-4 w-full flex-wrap md:flex-row justify-between">
        {populars.map((popular, index) => (
          <div
            key={index}
            className="people flex flex-col basis-full justify-center items-center md:basis-[45%] lg:basis-[33%] xl:basis-[24%]  px-4 py-6 text-white gap-3 lg:gap-2 relative"
          >
            <img className="flex rounded-md " src={popular.image} alt="" />
            <div className="flex-flex-col bg-[#011631] px-12 py-3 absolute bottom-16 justify-center items-center">
              <p className="text-[20px]">{popular.art}</p>
              <p className="hover:text-[#009fed] ease-in-out text-[16px]">
                {popular.item}
              </p>
            </div>
          </div>
